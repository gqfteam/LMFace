...
public class MyStoreOrderListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public MyStoreOrderListAdapter(Context mContext, List<order_goods_usermsg> mDatas) {
    ...
    public void setCourierNum(String courier,int position){
        Subscription subscription = NetWork.getGoodsOrderService().updateOrderStatusAndCourierNum(datas.get(position).getOrderId(),2,courier)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<ResultCode>() {
                    @Override
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int p) {
        ...
        mHolder.myOrderStoreUserName.setText("");
            ...
            case 0:
                ...
                mHolder.commitCancelGoodsBtn.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        //取消订单
                        showDialog("是否取消订单",p,-1);
                    }
                });
                mHolder.commitGetGoodsLin.setVisibility(View.VISIBLE);
                mHolder.commitGetGoodsBtn.setText("点击确认");
                mHolder.commitGetGoodsBtn.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        //确认订单
                        showDialog("是否确认订单",p,1);
                    }
                });
                break;
            case 1:
                mHolder.myOrderStatu.setText("等待发货");
                mHolder.commitCancelGoodsBtn.setVisibility(View.GONE);
                mHolder.commitGetGoodsLin.setVisibility(View.VISIBLE);
                mHolder.commitGetGoodsBtn.setText("点击输入快递号");
                mHolder.commitGetGoodsBtn.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        //弹出dialog输入快递号
                       if(mItemClickListener!=null){
                           mItemClickListener.showDialog(p);
                       }
                    }
                });
                break;
            case 2:
                mHolder.myOrderStatu.setText("等待签收");
                mHolder.commitGetGoodsLin.setVisibility(View.GONE);
